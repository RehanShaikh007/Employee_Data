...
const EmplMng = () =>{
    ...
    const removeFromTeam = (teamMember) =>{
        setEmployees ([...employees, teamMember]);
        setTeamMembers(teamMembers.filter((tm) => tm.id !== teamMember.id));
    }

    const calculateAverageAge = () => {
        if (teamMembers.length > 0) {
            const totalAge = teamMembers.reduce((sum, tm) => sum + tm.age, 0);
        ...
        <div className="employee-list">
            ...
            <ul>
              {employees.map((emp) => (
                <li key={emp.id}>
                    <div className="list">
                    <h4>{emp.first_name}</h4> {emp.age}
                    </div>    
                  <button onClick={() => addToTeam(emp)} disabled={teamMembers.includes(emp)}>
                    ADD
                  </button>
                </li>
              ))}
          ...
          <div className="team-list">
            <h2>Team Members</h2>
            <button className="sortBtn" onClick={sortTeamMembersByAge}>SORT BY AGE</button>
            <ul>
              {teamMembers.map((tm) => (
                <li key={tm.id}>
                    <div className="list">
                  <h4>{tm.first_name}</h4> {tm.age}
                    </div>
                  <button className="removeBtn" onClick={() => removeFromTeam(tm)}>REMOVE</button>
                </li>
              ))}
            </ul>
          </div>
